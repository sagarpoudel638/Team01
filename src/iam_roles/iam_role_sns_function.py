...
def get_sns_topic_subscriptions(topic_arns):
    ...
    for topic_arn in topic_arns:
        ...
        try:
            ...
            lambda_subs = [sub for sub in subscriptions['Subscriptions'] if sub['Protocol'] == 'lambda']
...
def push_metrics_to_prometheus_pushgateway(sns_costs):
    ...
    gauge = Gauge('aws_sns_cost', 'Cost of AWS SNS by Topic ARN', ['topic_arn'], registry=registry)
    
    for topic_arn, cost in sns_costs.items():
        gauge.labels(topic_arn=topic_arn).set(cost)
        # Uncomment and replace 'your_pushgateway_address' with your actual Pushgateway address
        # push_to_gateway('your_pushgateway_address', job='sns_costs', registry=registry)
...
def lambda_handler(event, context):
    ...
    sns_cost_details = [{"Topic ARN": arn, "Cost": f"${cost:.2f}"} for arn, cost in sns_costs.items()]
