...
def lambda_handler(event, context):
    ...
    cost_data = generate_cost_data(iam_roles, lambda_role_mapping, bucket_name, file_key)
...
def generate_cost_data(iam_roles, lambda_role_mapping, bucket_name, file_key):
    ...
    for row in csv_reader:
        if row["lineItem/ProductCode"] == "AmazonCloudWatch":
            ...
            role_arn = extract_role_arn_from_resource_id(resource_id, lambda_role_mapping)
            if role_arn:
...
def extract_role_arn_from_resource_id(resource_id, lambda_role_mapping):
    ...
    if len(parts) >= 7 and parts[2] == "logs" and parts[5] == "log-group" and parts[6].startswith("/aws/lambda/"):
...
def push_metrics_to_prometheus_pushgateway(cost_data):
    ...
    gauge = Gauge('aws_cloud_cost', 'Cost of AWS Resources by Role', ['role'], registry=registry)
    for role, cost in cost_data.items():
        gauge.labels(role=role).set(cost)
    # Push the metrics to the Pushgateway
    push_to_gateway(
        os.environ["prometheus_ip"], job="aws_cloud_cost", registry=registry
    )
